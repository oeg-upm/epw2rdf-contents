...
def removeFileTTL(document):
    if os.path.exists("converter/DataStorage/" + document + "_EPW.r2rml.ttl"):
        os.remove("converter/DataStorage/" + document + "_EPW.r2rml.ttl")
        return
    else:
        return
...
def createFileTTL(document):
    documentCTD=open("converter/DataStorage/" + document + "_EPW.r2rml.ttl", "a+")
